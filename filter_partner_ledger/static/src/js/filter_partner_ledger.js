    ...
    var accountReportsWidget = AbstractAction.extend(ControlPanelMixin, {
        ...
        custom_events: {
            'value_changed': function(ev) {
                ...
                self.report_options.paymentterm_ids = ev.data.paymentterm_ids;
        ...
        render_searchview_buttons: function() {
            ...
            if (this.report_options.partner) {
                if (!this.M2MFilters) {
                    ...
                    if ('paymentterm_ids' in this.report_options) {
                        fields['paymentterm_ids'] = {
                            label: _t('Payment Terms'),
                            modelName: 'account.payment.term',
                            value: this.report_options.paymentterm_ids.map(Number),
                        };
                    }
                    if (!_.isEmpty(fields)) {
