...
class CustomerNumberPartnerLedger(models.AbstractModel):
    ...
    def _get_lines(self, options, line_id=None):
        ...
        sorted_partner_num = sorted(grouped_partners, key=lambda p: p.partner_number or '')#sorted the partner according to partner_number
        # unfold_all = context.get('print_mode') and not options.get(
        #     'unfolded_lines')
        total_initial_balance = total_debit = total_credit = total_balance = 0.0
        for partner in sorted_partner_num:
            ...
            columns = [self.format_value(initial_balance),
                       self.format_value(debit), self.format_value(credit)]

            if self.user_has_groups('base.group_multi_currency'):
                columns.append('')
            columns.append(self.format_value(balance))
            # columns.append(partner.partner_number)
            ...
            if offset == 0:
                lines.append({
                    ...
                    'trust': partner.trust,
                    'unfoldable': True,
                    'unfolded': 'partner_' + str(partner.id) in options.get(
                        'unfolded_lines'),
                    'colspan': 6,
                })
            ...
            if 'partner_' + str(partner.id) in options.get(
                    'unfolded_lines'):
                if offset == 0:
                ...
                for line in amls:
                    ...
                    line_debit = line_currency._convert(line_debit, used_currency,
                                                        user_company, date)
                    line_credit = line_currency._convert(line_credit,
                                                         used_currency,
                                                         user_company, date)
                    ...
                    domain_columns = [line.journal_id.code, line.account_id.code,
                                      ...
                                      line.date_maturity and format_date(self.env,
                    ...
                    if self.user_has_groups('base.group_multi_currency'):
                        domain_columns.append(
                            self.with_context(no_format=False).format_value(
