...
class FollowupReportStatement(models.Model):
    _name = 'report.statement'
    _auto = False
    _description = 'Followup Report Statement'
    _order = 'car_id'
    ...
    @api.model_cr
    def init(self):
        tools.drop_view_if_exists(self.env.cr, 'report_statement')
        self.env.cr.execute("""CREATE OR REPLACE VIEW public.report_statement AS
           ...
           FROM ( SELECT sum(bf.amount_total_signed) AS bforward
                ...
                UNION
                 SELECT sum(movl.balance_cash_basis) AS bforward
                  ...
                  WHERE pay.state::text <> 'cancel'::text AND pay.state::text <> 'draft'::text AND pay.partner_id = inv.partner_id AND movl.account_id = 8) x) AS bforward,
    inv.partner_number,
    inv.name,
    inv.amount_total,
    inv.outstanding,
