...
class AccountInvoice(models.Model):
    ...
    def _compute_amount(self):
        ...
        self.amount_total = int(self.amount_untaxed + self.amount_tax) if self.type == 'out_invoice' and self.payment_term_id.id not in [3, 12] else (
    ...
    def _compute_residual(self):
        ...
        self.residual_company_signed = int(abs(residual_company_signed) * sign) if self.payment_term_id.id not in [3,
                                                                                                                   12] else abs(
            residual_company_signed) * sign
        self.residual_signed = int(abs(residual) * sign) if self.type == 'out_invoice' and self.payment_term_id.id not in [3, 12] else abs(
            residual) * sign
        self.residual = int(abs(residual)) if self.type == 'out_invoice' and self.payment_term_id.id not in [3, 12] else abs(residual)
    ...
    def action_move_create(self):
        ...
        for inv in self:
            ...
            if inv.payment_term_id:  # Extra added code, to resolve the round off issue, If any issues remove this code from here 'ESTART' to 'ESTOP'.
                if self.type == 'out_invoice' and inv.payment_term_id.id not in [3, 12]:
                    if not any(check_line[2]['quantity'] < 0 for check_line in line if check_line[2]):
                        check_credit = 0
                        check_debit = 0         # A temporary code is adding
                        add_val = 0             # Find very hard to solve a issue that happend in this time, thats why this kind of code is added.
                        dec_val = 0
                        large_val = 0
                        for check_line in line:
                            if check_line[2]:
                                if check_line[2]['debit']:
                                    ...
                                    check_debit = check_line[2]['debit']
                                if check_line[2]['credit']:
                                    try:
                                        val = str(check_line[2]['credit']).split(".")[1]
                                        if int(val) > dec_val:
                                            dec_val = int(val)
                                            large_val = int(check_line[2]['credit'])
                                    except:
                                        print("Error !")
                                        
                                    check_line[2]['credit'] = int(check_line[2]['credit'])
                                    check_credit = check_credit + check_line[2]['credit']
                        if check_debit != 0:
                            if check_debit - check_credit != 0:
                                add_val = check_debit - check_credit
                        for check_line in line:
                            if check_line[2]:
                                if check_line[2]['credit']:
                                    if check_line[2]['credit'] == large_val:
                                        check_line[2]['credit'] += add_val
                                        break      # ESTOP : Remove code till here from ESTART if any issues.
    ...
    def _amount_all(self):
        ...
        for order in self:
            ...
            order.update({
                ...
                'roundof_discount': (amount_untaxed + amount_tax) - int(
                    amount_untaxed + amount_tax) if order.payment_term_id.id not in [3, 12] else 0,

                'amount_total': int(amount_untaxed + amount_tax) if order.payment_term_id.id not in [3, 12] else (
